    ...
    public class GenericCommand<T> : CommandBase
    {
        ...
        public override bool CanExecute(object parameter)
        {
            ...
            else if (_canExecute != null && parameter is T)
            {
                if (!DesignMode.DesignModeEnabled)
                {
                    ...
                    BasePlatform.CurrentCore.Logger.Log(LogLevels.Debug, "{0} - Return Value: {1}  Parameter: {2}", name, value, parameter);
                }
        ...
        public override void Execute(object parameter)
        {
            if (!DesignMode.DesignModeEnabled)
            {
                ...
                BasePlatform.CurrentCore.Analytics.Event(name, parameter);
                BasePlatform.CurrentCore.Logger.Log(LogLevels.Information, "{0} - Parameter: {1}", name, parameter);
            }
