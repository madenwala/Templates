...
namespace AppFramework.Core.ViewModels
{
    ...
    public abstract class CollectionViewModelBase : ViewModelBase
    {
        ...
        protected override async Task OnSaveStateAsync(SaveStateEventArgs e)
        {
            ...
            {
                PlatformCore.Current.Logger.LogError(ex, "Error during CollectionViewModelBase.OnSaveStateAsync calling each individual child ViewModel.SaveStateAsync");
                throw;
            }
        ...
        internal async Task SetCurrentAsync(ViewModelBase vm)
        {
            ...
            if (this.CurrentViewModel != null)
            {
                PlatformCore.Current.Logger.Log(LogLevels.Debug, "CollectionViewModelBase.SetCurrent to {0}", vm);
                this.CurrentViewModel.PropertyChanged += CurrentVM_PropertyChanged;
