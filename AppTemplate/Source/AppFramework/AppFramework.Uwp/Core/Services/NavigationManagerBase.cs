...
namespace AppFramework.Core.Services
{
    public abstract class NavigationManagerBase : ServiceBase, IServiceSignout
    {
        ...
        public void HandleActivation(IActivatedEventArgs e, Frame rootFrame)
        {
            ...
            try
            {
                PlatformCore.Current.Analytics.Event("HandleActivation", e.Kind);

                switch (e.Kind)
                {
                    case ActivationKind.Launch:
                        var launchArg = e as LaunchActivatedEventArgs;
                        PlatformCore.Current.Logger.Log(LogLevels.Warning, "Calling OnActivation({0})  TileID: {1}  Arguments: {2}", e?.GetType().Name, launchArg.TileId, launchArg.Arguments);
                        handled = this.OnActivation(launchArg);
                        break;

                    case ActivationKind.VoiceCommand:
                        var voiceArgs = e as VoiceCommandActivatedEventArgs;
                        PlatformCore.Current.Logger.Log(LogLevels.Warning, "Calling OnActivation({0})", e?.GetType().Name);
                        handled = this.OnActivation(voiceArgs);
                        break;

                    case ActivationKind.ToastNotification:
                        var toastArgs = e as ToastNotificationActivatedEventArgs;
                        PlatformCore.Current.Logger.Log(LogLevels.Warning, "Calling OnActivation({0})  Arguments: {1}", e?.GetType().Name, toastArgs.Argument);
                        handled = this.OnActivation(toastArgs);
                        break;

                    case ActivationKind.Protocol:
                        var protocolArgs = e as ProtocolActivatedEventArgs;
                        PlatformCore.Current.Logger.Log(LogLevels.Warning, "Calling OnActivation({0})  Arguments: {1}", e?.GetType().Name, protocolArgs.Uri.ToString());
                        handled = this.OnActivation(protocolArgs);
                        break;

                    default:
                        PlatformCore.Current.Logger.LogError(new Exception(string.Format("Can't call OnActivation({0}) as it's not implemented!", e.Kind)));
                        handled = false;
                        break;
                }
                ...
                PlatformCore.Current.Logger.Log(LogLevels.Information, "Completed Navigation.HandleActivation({0}) on RootFrame: {1} --- OnActivation Handled? {2}", e?.GetType().Name, rootFrame?.Content?.GetType().Name, handled);
            }
            catch (Exception ex)
            {
                PlatformCore.Current.Logger.LogError(ex, "Error during App Navigation.HandleActivation({0}) on RootFrame: {1}", e?.GetType().Name, rootFrame?.Content?.GetType().Name);
                throw ex;
            }
        }
        ...
        private bool OnActivation(LaunchActivatedEventArgs e)
        {
            ...
            if (handled == false && PlatformCore.Current.InitializationMode == InitializationModes.Restore)
                handled = true;
        ...
        {
            return this.HandleArgumentsActivation(e.Uri.AbsoluteUri.Replace(PlatformCore.Current.AppInfo.ProtocolPrefix, "").Split(':').Last());
        }
        ...
        private bool HandleArgumentsActivation(string arguments)
        {
            ...
            PlatformCore.Current.Logger.Log(LogLevels.Information, "HandleArgumentsActivation: {0}", arguments);
            ...
            {
                PlatformCore.Current.Logger.LogError(ex, "Could not parse argument '{0}' passed into app.", arguments);
                return false;
            }
        }
        ...
        public void Exit()
        {
            PlatformCore.Current.Analytics.Event("ApplicationExit");
        ...
        public async Task SignoutAsync()
        {
            ...
            {
                PlatformCore.Current.Logger.LogError(ex, "Could not close all secondary windows on Signout!");
                throw ex;
            }
        }
        ...
        public void MapExternal(ILocationModel loc, string label = null, MapExternalOptions mapOption = MapExternalOptions.Normal, double zoomLevel = 16)
        {
            ...
            PlatformCore.Current.Analytics.Event("MapExternal-" + mapOption.ToString());
        ...
        {
            PlatformCore.Current.Analytics.Event("NavigateToWebBrowser");
        ...
        {
            PlatformCore.Current.Analytics.Event("NavigateToWebView");
        ...
        public void NavigateToTwitterScreenName(string screenname)
        {
            ...
            PlatformCore.Current.Logger.Log(LogLevels.Warning, $"Launching Twitter @{screenname} -- {url}");
        ...
        public virtual async Task NavigateToNewWindow(Type viewType, object parameter)
        {
            try
            {
                ...
                await view.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () =>
                {
                    ...
                    PlatformCore.Current.Logger.Log(LogLevels.Warning, $"Launched new window");
                });
            ...
            {
                PlatformCore.Current.Logger.LogError(ex, "Could not create new window for view type {0}.", viewType.Name);
                throw ex;
            }
        }
        ...
        private void View_Consolidated(ApplicationView sender, ApplicationViewConsolidatedEventArgs args)
        {
            ...
            PlatformCore.Current.Logger.Log(LogLevels.Debug, $"Closed secondary window with ID {windowID}");
        ...
        public CommandBase NavigateToFeedbackCommand
        {
            get
            {
                return _navigateToFeedbackCommand ?? (_navigateToFeedbackCommand = new GenericCommand("NavigateToFeedbackCommand", async () =>
                {
                    if (Windows.Foundation.Metadata.ApiInformation.IsTypePresent("Microsoft.Services.Store.Engagement.StoreServicesFeedbackLauncher"))
                    {
                        PlatformCore.Current.Analytics.Event("FeedbackLauncher");
        ...
        {
            PlatformCore.Current.Logger.Log(LogLevels.Debug, "Hardware back button pressed.");
            e.Handled = this.GoBack();
        }
        ...
        private void CoreDispatcher_AcceleratorKeyActivated(CoreDispatcher sender, AcceleratorKeyEventArgs e)
        {
                ...
                )
            {
                ...
                {
                    PlatformCore.Current.Logger.Log(LogLevels.Warning, $"Windows accelerator keyboard key pressed to go back: {virtualKey}");
                    e.Handled = this.GoBack();
                }
                    ...
                    (virtualKey == VirtualKey.Right && onlyAlt))
                {
                    ...
                    PlatformCore.Current.Logger.Log(LogLevels.Warning, $"Windows accelerator key pressed to go forward: {virtualKey}");
        ...
        private void CoreWindow_PointerPressed(CoreWindow sender, PointerEventArgs e)
        {
            ...
            if (backPressed ^ forwardPressed)
            {
                if (backPressed)
                {
                    PlatformCore.Current.Logger.Log(LogLevels.Warning, "Windows accelerator mouse key pressed to go back.");
                    e.Handled = this.GoBack();
                }
                if (forwardPressed)
                {
                    PlatformCore.Current.Logger.Log(LogLevels.Warning, "Windows accelerator mouse key pressed to go forward.");
                    e.Handled = this.GoForward();
                }
            }
        }
